...
\keywords{DNA copy number, dynamic programming, joint segmentation, parralel computing}
\end{abstract}
...
\paragraph{Proof.} 
%We can always write:
%$$ {C}_{(r)}^{(1)} = \widetilde{C}_{(r)}^{(1)} - \log\left(\sum_p \pi_p \exp \left\{ -{\sum_{t \in r} (y(t)- \mu_p)^2 }/{ 2 \sigma^2} +\widetilde{C}_{(r)}^{(1)} \right\}\right)$$
%Moreover we have:
%$$-\log(p) \ \leq \ 
%- \log\left(\sum_p \pi_p \exp \left\{ -{\sum_{t \in r} (y(t) - \mu_p)^2 }/{ 2 \sigma^2} +\widetilde{C}_{(r)}^{(1)} \right\}\right)
%\ \leq \  
%0$$

...
All $C^{(k)}_{1,1}(\mu_p)$ are initialized as $+\infty$ and $C^{(1)}_{1,t}$ are initialized using their definition. At step $k, t$ of Algorithm \ref{algo:DPALinear}  $\mathcal{O}(P)$ basic operations are performed. If we sum these for all $k < K$ and $t < n$  we straighfowardly see that the algorithm has an $\mathcal{O}(KPn)$ time complexity.
\begin{algorithm}
\begin{algorithmic}
...
\end{theo}
\vspace{-1cm}
\noindent $$\text{\textit{Proof.} Since } {C}_{(r)}^{(1)} = \widetilde{C}_{(r)}^{(1)} - \log\left(\sum_p \pi_p \exp \left\{ -\sum_{t \in r} (y(t)- \mu_p)^2 / 2 \sigma^2 +\widetilde{C}_{(r)}^{(1)} \right\}\right),$$
$$\text{and }-\log(p) \ \leq \ - \log\left(\sum_p \pi_p \exp \left\{ -\sum_{t \in r} (y(t) - \mu_p)^2 / 2 \sigma^2 +\widetilde{C}_{(r)}^{(1)} \right\}\right)\ \leq \  
0,$$ we have: $\forall r,\, \widetilde{C}_{(r)}^{(1)} -\log(p) \leq {C}_{(r)}^{(1)} \leq \widetilde{C}_{(r)}^{(1)} \label{equation:bound1}$, which gives, along with the definition of ${C}_{(R)}^{(K)}$: 
%\begin{eqnarray} 
$
\forall m \in \mathcal{M}^{(K)}_{(R)}, \,
\widetilde{C}_{(R)}^{(K)} - K\log(p) \leq \sum_{r \in m} \widetilde{C}_{(r)}^{(1)} - K \log(p) \leq   \sum_{r \in m} {C}_{(r)}^{(1)} (2).
...
%\begin{eqnarray} 
$
\forall m \in \mathcal{M}^{(K)}_{(R)}, \, {C}_{(R)}^{(K)} \leq \sum_{r \in m} {C}_{(r)}^{(1)} \leq   \sum_{r \in m} \widetilde{C}_{(r)}^{(1)} (3).
\label{equation:boundproof2}
%\end{eqnarray}
$
Applying Equation (2) to $m = \widehat{m}_{t_1, t_2}^{(K)}$ and then Equation (3) to $m =  \widetilde{m}_{t_1, t_2}^{(K)}$ we get the theorem. $\blacksquare$
